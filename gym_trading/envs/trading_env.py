    ...
    def step(self, action):
        ...
        if self.market_position == MarketPosition.Flat:
            if self.space[action] == "buy":
                ...
                self.stoploss = self.price_entry - round(self.stoploss_ticks) * settings.SYMBOL_LIST[self.symbol][
                    'tickSize']
...
class TradingEnv(gym.Env):
    ...
    def __init__(self, symbol=None, start_date=None, end_date=None, info_st=None, info_lt=None, enable_tick_by_tick_step=False, use_settings=True, connection: HistoricalConnection=None, transposed=True, jump_date=False):
        ...
        self.observation_space = gym.spaces.Dict({
                                        ...
                                        #'upper_pivots': gym.spaces.Box(low=0, high=1, shape=(len(self.market.bars_array),), dtype=np.float),
                                        #'lower_pivots': gym.spaces.Box(low=0, high=1, shape=(len(self.market.bars_array),), dtype=np.float),
                                        'market_position': gym.spaces.Box(low=0, high=1, shape=(len(MarketPosition),), dtype=np.int),
                                        'stoploss': gym.spaces.Box(low=0, high=1, shape=(1,), dtype=np.float),
                                        'available_actions': gym.spaces.Box(low=0, high=1, shape=(self.action_space.n,), dtype=np.int),
                                        'available_actions_one_hot': gym.spaces.Box(low=0, high=1, shape=(self.action_space.n,), dtype=np.int)
        })
    ...
    def get_available_actions_one_hot(self):
        actions = None
        available_actions = np.zeros(shape=(self.action_space.n,), dtype=np.int)

        if self.market.market_position == MarketPosition.Flat:
            actions = ['buy', 'sell', 'nothing']
            for i, act in enumerate(self.market.actions):
                if act[0] in actions:
                   available_actions[i] = 1
        elif self.market.market_position == MarketPosition.Long:
            ticks = int((self.market.current_price - self.market.stoploss - settings.SYMBOL_LIST[self.symbol]['tickSize']) / settings.SYMBOL_LIST[self.symbol]['tickSize'])
            actions = ["nothing", "close", *["tu{}".format(index + 1) for index in range(ticks)]]
            for i, act in enumerate(self.market.actions):
                if act[0] in actions:
                   available_actions[i] = 1
        elif self.market.market_position == MarketPosition.Short:
            ticks = int((self.market.stoploss - self.market.current_price - settings.SYMBOL_LIST[self.symbol]['tickSize']) / settings.SYMBOL_LIST[self.symbol]['tickSize'])
            actions = ["nothing", "close", *["td{}".format(index + 1) for index in range(ticks)]]
            for i, act in enumerate(self.market.actions):
                if act[0] in actions:
                   available_actions[i] = 1

        #print("AVAILABLE ACTIONS ONE HOT ENCODER")
        #print(available_actions.reshape(1, *available_actions.shape))
        #print(available_actions)

        return available_actions

    def get_available_actions_embedding(self):
        available_actions = []
        available_actions.append("")

        if self.market.market_position == MarketPosition.Flat:
            for t in ['buy', 'sell', 'nothing']:
                available_actions[0] += "{} ".format(t)
        elif self.market.market_position == MarketPosition.Long:
            ticks = int((self.market.current_price - self.market.stoploss - settings.SYMBOL_LIST[self.symbol]['tickSize']) / settings.SYMBOL_LIST[self.symbol]['tickSize'])
            for t in ["nothing", "close", *["tu{}".format(index) for index in range(1, ticks) if index <= self.market.stoploss_ticks]]:
                available_actions[0] += "{} ".format(t)
        elif self.market.market_position == MarketPosition.Short:
            ticks = int((self.market.stoploss - self.market.current_price - settings.SYMBOL_LIST[self.symbol]['tickSize']) / settings.SYMBOL_LIST[self.symbol]['tickSize'])
            for t in ["nothing", "close", *["td{}".format(index) for index in range(1, ticks) if index <= self.market.stoploss_ticks]]:
                available_actions[0] += "{} ".format(t)

        #print(available_actions, " --- ", self.market.market_position, " --- ", self.market.current_price, " --- ", self.market.stoploss)
        result = self.action_token.texts_to_sequences(available_actions)
        #print(result)
        result = pad_sequences(result, maxlen=self.action_space.n, padding='post')
    ...
    def step(self, action):
        ...
        available_actions = self.get_available_actions_names()

        if name_action not in available_actions:
            print(available_actions)
            print('La accion elegida no esta en las disponibles.')
    ...
    def render(self, mode='human'):
        ...
        print("*** Account Balance: {} *** Profit / Loss: {}".format(self.market.account_balance, profit_loss))
        print("*** Account Balance + P/L: {}".format(self.market.account_balance + profit_loss))
        print("*** Reward Function: {}".format(self.current_reward))
        print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        print("*** {:<20} {:>12} {:>12} {:>12}".format("PERFORMANCE", "TOTAL", "LONG", "SHORT"))
        print("*** {:<20} {:>12} {:>12} {:>12}".format("Total Net Profit",
                                                       self.market.get_net_profit(),
                                                       self.market.get_net_profit(MarketPosition.Long),
                                                       self.market.get_net_profit(MarketPosition.Short)))
        print("*** {:<20} {:>12} {:>12} {:>12}".format("Gross Profit",
                                                       self.market.get_gross_profit(),
                                                       self.market.get_gross_profit(MarketPosition.Long),
                                                       self.market.get_gross_profit(MarketPosition.Short)))
        print("*** {:<20} {:>12} {:>12} {:>12}".format("Gross Loss",
                                                       self.market.get_gross_loss(),
                                                       self.market.get_gross_loss(MarketPosition.Long),
                                                       self.market.get_gross_loss(MarketPosition.Short)))
        print("*** {:<20} {:>12} {:>12} {:>12}".format("Commisions",
                                                       self.market.get_commisions(),
                                                       self.market.get_commisions(MarketPosition.Long),
                                                       self.market.get_commisions(MarketPosition.Short)))
